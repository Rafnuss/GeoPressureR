...
test_that("check graph_set_movement() with groundspeed/gamma", {
  ...
  expect_no_error(graph_set_movement(graph, power2prob = \(power) {
    (1 / power)^6
  }))
})
...
test_that("check speed2prob() with groundspeed/logis", {
  ...
  expect_no_error(graph_set_movement(
    graph,
    shape = 8,
    scale = 2,
    method = "logis"
...
test_that("check speed2prob() with airspeed/bird", {
  ...
  expect_no_error(graph_set_movement(graph, bird = bird, power2prob = \(power) {
    (1 / power)^6
  }))
