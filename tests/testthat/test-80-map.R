...
stap = data.frame(
  stap_id = 1:10,
  start = seq(as.POSIXct("2023-01-01", tz="UTC"), as.POSIXct("2023-01-10 UTC", tz="UTC"), by="day")
)
stap$end = stap$start + sample(1:10)*10000

test_that("map_create() | basic", {
  expect_error(map_create())
  expect_no_error(map <- map_create(data = data,
                             extent = extent,
                             scale = scale,
                             stap = stap,
                             id = "18LX",
  ...
  expect_no_error(map_create(data = data,
                             extent = extent,
                             scale = scale,
                             stap = stap))
...
test_that("map_create() | missing stap", {
  ...
  expect_no_error(map <- map_create(data = data_tmp,
                                    extent = extent,
                                    scale = scale,
                                    stap = stap))
})
