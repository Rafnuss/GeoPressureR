...
#' tag_read("18LX")
#'
#' tag_read("18LX", crop_start = "2017-08-01", crop_end = "2017-08-05")
#'
...
tag_read <- function(id = NULL,
  ...
  if (is.null(tag) & !is.null(id)){
    ...
    if (is.character(tag)){
      cli::cli_abort(c(
        "x" = "{deparse(substitute(tag))} is a {.cls character} instead of a {.cls tag}.",
        ">" = "Make sure you provide the argument name `id = ` if you want to use {.fun read_tag} \\
        from an {.var id}"
      ))
    }
    # Check that id is not provided
    if (!is.null(id)){
      cli::cli_abort(c(
        "x" = "Both {.var tag} and {.var id} was provided in {.fun tag_read}",
        ">" = "Either of them need to be provided."
      ))
    }
  }
  ...
  if (!is.na(sensor_paths[[2]])) {
    tag$light <- switch(tools::file_ext(sensor_paths[[2]]),
      "glf" = {
        subset(tag_read_dto(sensor_paths[[2]], quiet = quiet), date >= crop_start & date < crop_end)
      },
      "lux" = {
  ...
  if (!is.na(sensor_paths[[3]])) {
    tag$acceleration <- switch(tools::file_ext(sensor_paths[[3]]),
      "acceleration" = {
        subset(tag_read_dto(sensor_paths[[3]], col = 4, quiet = quiet), date >= crop_start & date < crop_end)
      },
      "deg" = {
        # Check that it is a valid Migrate Technology file
...
#' @family tag
#' @noRd
tag_read_dto <- function(sensor_path, skip = 6, col = 3, date_format = "%d.%m.%Y %H:%M", quiet = FALSE) {
