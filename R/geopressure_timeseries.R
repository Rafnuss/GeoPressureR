...
#' `geopressure_timeseries_latlon()`. The computation is optimized by paralleling the call of
...
#' flight altitude.
#'
#' Read more about how this is computed with [`geopressure_timeseries_latlon()`]
#'
...
#' computed with `geopressure_timeseries_latlon()`:
...
#' )
#' @export
geopressure_timeseries <- function(path,
                                   pressure,
  ...
  for (i_s in seq_len(nrow(path))) {
    ...
    # Send the query
    f[[i_s]] <- future::future({
      geopressure_timeseries_latlon(path$lat[i_s], path$lon[i_s],
