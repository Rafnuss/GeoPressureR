...
#' @export
geolight_light2mat <- function(light, twl_offset = 0) {
  ...
  date <- seq(
    from = as.POSIXct(format(light$date[1] - twl_offset, "%Y-%m-%d"), tz = "UTC"),
    to = as.POSIXct(format(light$date[length(light$date)] - twl_offset, "%Y-%m-%d"),
      tz = "UTC"
    ) + 60 * 60 * 24 - res,
    by = res
  )
  date <- date + twl_offset
