...
#' `geostap2path`, `graph_most_likely`, `graph_simulation` and `geopressureviz`.
#'
#' @param ind index in the 2D map (vector or matrix)
#' @param geostap_graph either a `geostap` or a `graph` which contains both `stap`, `extent` and
#' `scale`.
...
#' geostap <- geostap_create(tag, c(-16, 23, 0, 50), scale = 1)
#'
...
ind2path <- function(ind,
                     geostap_graph) {
  assertthat::assert_that(is.list(geostap_graph))
  assertthat::assert_that(assertthat::has_name(geostap_graph, "stap"))
  stap <- geostap_graph$stap
  ...
  assertthat::assert_that(assertthat::has_name(geostap_graph, "scale"))
  assertthat::assert_that(assertthat::has_name(geostap_graph, "extent"))
  g <- geo_expand(geostap_graph$extent, geostap_graph$scale)
