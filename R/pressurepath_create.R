...
#' @description
#'
#' A `pressurepath` is a data.frame merging `path` and `tag$pressure`. It can be thought as the equivalent
#' of the measurement of variables by sensors equipped on the bird moving along a specific `path`.
#'
#' Any ERA variables can be retrieve but the functions is most notably used to retrieve ERA5 surface pressure
#' to then be able to compare it to the pressure measured by the tag, and potentially adjust labeling tag data.
#'
...
#'
#' Positions during the flight are estimated by linear interpolation of the position of the stap before and after.
#' `pressurepath_create()` does not return measurement for stationary periods which are not provided in `path` as well
#' as the flight before and after.
#'
#' `pressurepath_create()` also return the altitude of the bird along its trajectory. The altitude \eqn{z_{tag}} (a.s.l.) is computed
#' from the tag pressure \eqn{P_{tag}}, using the barometric equation
#' \deqn{ z_{{tag}}(x)=z_{ERA5}(x) + \frac{T_{ERA5}(x)}{L_b}  \left( \frac{P_{tag}}{P_{ERA5}(x)}
...
#' - `surface_pressure_norm` pressure retrieved from ERA5 normalized to the average of `pressure_tag`
...
pressurepath_create <- function(tag,
  ...
  id  = ceiling(stap_id_interp) %in% path$stap_id & floor(stap_id_interp) %in% path$stap_id
