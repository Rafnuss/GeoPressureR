...
plot_pressurepath <- function(pressurepath,
  ...
  tag_era5 <- merge(
    stats::aggregate(list(error_sd = pp$error), list(stapelev = pp$stapelev), \(x) round(stats::sd(x), 2)),
  ...
  if ("ts" == type) {
    # Remove flight from warning
    id <- pp$stap_id > 0
    pp$warning <- F
    pp$warning[id] <- error_warning(pp$error[id], pp$error_sd[id], pp$stap_id[id])
    ...
    id <- pp$stap_id > 0
    pp$sd_param <- 0
    pp$sd_param[id] <- tag$param$sd[pp$stap_id[id]]
    pp$sd_ok <- T
    pp$sd_ok[id] <- pp$error_sd[id] > tag$param$sd[pp$stap_id[id]]
    ...
    p <- ggplot2::ggplot(pp[pp$stap_id != 0, ], ggplot2::aes(x = error)) +
      ggplot2::geom_histogram(
        ggplot2::aes(
          y = (ggplot2::after_stat(count)) / tapply(ggplot2::after_stat(count), ..PANEL.., sum)[..PANEL..],
          fill = !sd_ok
        ),
        binwidth = .4
      ) +
