...
write.csv(
  data.frame(
    ...
    value =       value = (as.numeric(format(twl$twilight, "%H")) * 60 + as.numeric(format(twl$twilight, "%M")) - shift_k/60 + 60 * 12) %% (60*24),
...
Aggregating the probability map of each twilight per stationary period requires some assumptions on the independence/correlation of the twilight errors. Read more about this in the vignette [Probability-aggregation](probability-aggregation.html)). Here, we use a log-linear pooling with a weight of $w=0.1$,
