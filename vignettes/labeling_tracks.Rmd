...
In this vignette, we'll explore how to label your track files (activity and pressure), and I'll try to provide some useful tips to make that smoother, faster and more successful. 

If don't understand where , see an example of the overall process in the section ["Editing on TRAINSET" of basic example](basic_example.html#editing-on-trainset)

## Why a manual labeling?

First, why should I bother with a manual editing and not using an automatic function?

Put simply, **it' will be's faster**. Trying to get any algorithm tuned to each specific species  will take more time than going through them manually. You can expect to spend between 30sec (e.g. Mangrove Kingfisher) to 10min (e.g. Eurasian Nightjar) per tracks depending on the species migrating complexity. 

More than saving time, **it will give you a sense of what your bird is doing**. You'll get to know how birs is moving (e.g., long continuous high altitude flight, short ones over multiple days, alternation between short migration-stopover etc...). It will also give you an idea of the uncertainty of your classification. Is the bird really moving or is it just a changing altitude? All of that will be very useful to understand the result of the modeled track path. 

But the more important reason for doing that is that the pressure mapping required very highly accurately labelled activity and pressure data. Firstly activity labeling defined stationary period and flight duration. Short stationary period can be particularly hard to define, so that expert knowledge is essential. Flight duration has an important impact on the movement model so that for short successive stationary period (when pressure mapping is not precise), you really want to be accurate with the flight duration to have good esitmation of bird's position. Secondly, the pressure timeserie matching algorithm is very sensitive to erroneously labeled pressure, such that even a single mislabeled datapoint can through off the estimation map. 
...
The task is relatively simple: label migratory activity as `1` in the *accelaration* column and remove pressure outliar with `1` in the *pressure* column However, there are a lot of small loophole which can make this task actually difficult.

The purpose of labelisation of migratory activity is two-fold: 
- Define stationary period. During a stationary period, the bird is considered static relative to the size of the grid (~10-30km). The start and end of the stationary period will be used to define the timeseries of pressure which need to be matched. 
- Define flight duration. This information will be used in the movement model to define the distance between stationary period.
 
The purpose of labeling pressure has also two objectif;
- Eliminating outliar so that the match with ERA5 does not account for those datapoint.
- Eliminate pressure measurement corresponding when the bird was at a different elevation. 
...
- Trainest is offers more flexibility with the label than required: you can add and remove label value (bottom-right of the page. In order for `trainset_read()` to work, don't change/edit/add any label, simply use the one offered : "FALSE" and "TRUE".


## Sample test code

To help you out with this process, I've written this small script comprising of 3 basic test you can do to check what need to be labeled and that your label file is correct.
...
For this example, I'm using the first version exported from trainset
...
Note that the last row has a `next_flight_duration` of zero because it's the last stationary period. 
