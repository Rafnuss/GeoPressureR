...
Initialize the first path with the most likely position of each stationary period
...
Yet the position is unlikely to be reliable for short stationary period. To fix this, we will linearly interpolate the position of short stationary period. We first identify and remove the position of short stationary period (<24hr).
...
Then, on a slight tangent which need to be set here, we set the position of the first and last stationary period (equipment and retrieval) to the known site.
...
Finally, we can interpolate the position in latitude and longitude separately using the function `na.approx` from the `zoo` packages. A more elegant solution would be to account for the difference in flight duration. (Contact me if you have a better solution to this)

```{r}
path_ll_1 = arrayInd(path[1,], nll)
path_ll_1_inter = round(zoo::na.approx(path_ll_1))
...
```{r}
leaflet() %>% addTiles() %>% 
  addPolylines(lng=lon[path_ll_1[,2]], lat=lat[path_ll_1[,1]], opacity = 1, color = "#808080",weight = 3) %>% 
  addCircles(lng=lon[path_ll_1[,2]], lat=lat[path_ll_1[,1]], opacity = 1, color = "#000", weight = 10)
```
...
for (i in seq(from=50, to=nj,length.out=10)){
  ...
  m = m %>% addPolylines(lng=lon[path_ll[,2]], lat=lat[path_ll[,1]], opacity = 0.7, weight = 1, color = "#808080") %>% addCircles(lng=lon[path_ll[,2]], lat=lat[path_ll[,1]], opacity = 1, weight = 1, color = "#000")
}
m
```
