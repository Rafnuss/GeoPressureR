...
```
```{r, eval = F}
pressure_maps = geopressure_map(pam_data$pressure, extent=c(-16,20,0,50), scale=10, max_sample=100)
...
for (i_r in 1:length(pressure_prob)){
  ...
  # Remove mean
  pressure_timeserie[[i_r]]$pressure0 = pressure_timeserie[[i_r]]$pressure - mean(pressure_timeserie[[i_r]]$pressure) + mean(pam_pressure_sta$obs[!pam_pressure_sta$class])
}
```
...
for (i_r in seq_along(pressure_timeserie)){
  ...
  df3 <- merge(pressure_timeserie[[i_r]], subset(pam_data$pressure, !class & sta_id==i_s), by = "date")
