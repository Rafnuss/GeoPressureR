...
pam_data$sta$duration <- pam_data$sta$end - pam_data$sta$start
pam_data$sta$next_flight_duration <- c(0, difftime(tail( pam_data$sta$start, length( pam_data$sta$start)-1), head( pam_data$sta$end, length( pam_data$sta$end)-1),units = "hours"))
...
p <- ggplot() +
  ...
  geom_line(
    data = subset(pam_data$pressure, sta_id != 0),
...
We can estimate the probability map for each stationary period with the following code. We will cover these two functions in more details in the vignette [Pressure Map](articles/pressure-map.html).
...
for (i_r in seq_len(length(pressure_prob))) {
  ...
  # Visual check
  # library(leaflet)
  # leaflet() %>% addTiles() %>% addRasterImage(pressure_prob[[i_r]]) %>% addMarkers(lat=lat,lng=lon)

  # Subset the pressure timeserie for the period of the current stationary period
  pam_pressure_sta <- subset(pam_data$pressure, sta_id == i_s)

  # query the pressure at this location
  message("query:", i_r, "/", length(pressure_prob))
  pressure_timeserie[[i_r]] <- geopressure_ts(lon,
    lat,
    pressure = pam_pressure_sta
  )
  # Add sta_id
  pressure_timeserie[[i_r]]["sta_id"] <- i_s
  pressure_timeserie[[i_r]]["lon"] <- lon
  pressure_timeserie[[i_r]]["lat"] <- lat

  # Remove mean
  pressure_timeserie[[i_r]]$pressure0 <- pressure_timeserie[[i_r]]$pressure - mean(pressure_timeserie[[i_r]]$pressure) + mean(pam_pressure_sta$obs[!pam_pressure_sta$isoutliar])
}
```
...
p <- ggplot() +
  ...
  geom_line(data = do.call("rbind", pressure_timeserie), aes(x = date, y = pressure0, col = as.factor(sta_id))) +
  theme_bw() +
  scale_colour_manual(values = rep(RColorBrewer::brewer.pal(9, "Set1"), times = 4))
