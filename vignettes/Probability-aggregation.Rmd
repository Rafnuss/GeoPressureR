...
For each stationary period, we estimate the probability of the position of the bird $\textbf{x}$ based on a pressure timeseries measured by the geolocator $P_{gl}[t] \quad \forall t \in [1,\ldots,n]$, which in a Bayesian framework can be written as, 
...
$$F(\boldsymbol{\varepsilon}) \propto \prod_{t=1}^n f(\varepsilon[t])^{w_t} = \sum_{t=1}^n w_t \ell (\varepsilon[t])$$
using the log-likelihood notation $\ell=\log{f}$.
...
To quantify the correctness of the uncertainty estimate of the probability map produced at the calibration site, we quantify the error of an estimated position as the great circle distance to the most likely point. 

![](probability-aggregation-histogram.png){width=100%}
