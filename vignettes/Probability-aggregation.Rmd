...
At the core of GeoPressure, we try to estimate the position of the bird based on a pressure mismatch of the geolocator and the ERA5. A major challenge in this process is the transformation of map of errors (mismatch) into a probability.

In this vignette, we will explore this challenge, look at the theory behind it and explore possible solution. The Great Reed Warbler (18IC), we serve us as example to illustrate the solutions. 
...
We are interested here in determining the likelihood term $p(P_{gl} \mid \textbf{x})$ using quantify the probability of observing the timeserie $P_{gl}$ knowing that the bird is at location $\textbf{x}$. To quantify this probability, we look at the mismatch between $P_{gl}$ and the pressure timesrie of the ERA5 database $P_{ERA5}(\textbf{x})$ extracted at location $\textbf{x}$. We can reformulate the likelihood as as a probability function $f$ of an error term $\varepsilon(\textbf{x})$ which measure a distance between $P_{gl}$ and $P_{ERA5}(\textbf{x})$
$$p(P_{gl} \mid \textbf{x}) = f(\varepsilon[1],\ldots,\varepsilon[n])$$

This formulation of the problem help us to split our problem in two: first defining an error term $\varepsilon[t]$ and secondly defining the likelihood function $f$.

## Error term
In most cases, we would expect to measure the error term with a simple difference $P_{ERA5}(\textbf{x})[t]-P_{gl}[t].$ However, within a ERA5 grid cell of 9-30km, we can expect a wide range of altitude at which the bird can be location. As such there any offset between the two timeserie can be explained by the difference of altitude.

To solve this issue, we remove the mean pressure difference, essentially ignoring the absolute value of pressure (and altitude) such that the error term only look at the mismatch of the temporal variation, 
$$\varepsilon[t] =\right( P_{ERA5}(\textbf{x})[t]-P_{gl}[t]\right) - \left( \frac{1}{n}\sum_{i=1}^{n} P_{ERA5}(\textbf{x})[i]-P_{gl}[i] \right).$$

This way of building the error term has some important consequences. If you think of a timeseries of only one datapoint, the error term will be zero at all locations, resulting in an equally probable map. As the number of datapoint increase, the errors term will be more and more able to distinguish between "good" and "bad" location.
...
We are using [the GeoPressure API](https://raphaelnussbaumer.com/GeoPressureServer/#description) with the `geopressure_map()` function tom easure the mismatch of the pressure series. Because the API can realistically return only a single map per stationary period, it aggregatea the error timeseris with the Mean Square Error (MSE), 
...
We can post-process the data to get back some basic metadata, such as the number of datapoint of each stationay period used. We also compute the position with the minimum MSE
...
In order to find an appropriate likelihood function, we first need to assume a distribution of the error. The sources of the errors are (1) the sensor measurement error, (2) the ERA5 reanalysis error and (3) the attitudinal movement of the bird during this time. Because we arw removing the mean error, we can ignore any long-term errors (e.g., constant temporal error in ERA5 or biases in the geolocator sensor). 
...
![Histogram of the error term](./Appendix_F_4.png){width=100%}
 They all good close enough to a Gaussian distribution. In the case of the Great Reed Warbler, the standard deviation is around 0.5.
...
1. The first one is the absence of stationary. Indeed, the auto-covariance amplitude (and potnetially shape) is strongly influenced by the vertical displacement of the bird, which are strongly correlated with topography. For instance, the Eurasian Hoopoe has a high variance and large temporal range because it breeds at the feet of the Alps. During the rest of the year, it lives in rather flat area. Therefore, the covariance built from the breeding site would not be appropriate for the other sites. 
2. The second issue is in link with the definition of our error term and the different duration of stationary period. 
...
In the field of [proability aggreation](https://link.springer.com/article/10.1007/s11004-012-9396-3), the problem of combining sources of information with overlapping/redundant content is frame as finding a pooling operator $F$ such that,
...
This equation also relies on the principale of conjuction of probability, but it uses a weight $w_t$ which is related to the new information brought by each additional $\varepsilon[t]$. Formally, it can be defined by
$$w_t=\frac{\ln p(\varepsilon[t] \mid \boldsymbol{x},\varepsilon[1],\ldots \varepsilon[t-1])}{\ln p(\varepsilon[t] \mid \boldsymbol{x})}.$$
[Allard et al. (2012)](http://link.springer.com/10.1007/s11004-012-9396-3) is a great ressource to learn more about probability aggregation and log linear pooling.

The log-linear pooling aggregation can be simplify to the case of Gaussian independence when $w_t=1$. We have already explored this case earlier.
...
which can be interpreted as the Gaussian probability distributions of the MSE. The length of the timeserie $n$ has disappeared from the equation, so that the aggregation dependence only on the means square of the errors, regardless of how many datapoint.
...
This scheme was design to minimize the strength of $1/n$
...
Validation of uncertainty estimation is relatively difficult, specially when there are only a few datapoint and that you assume that the behaviour of the bird to be different at these calibration site (equipement and retrival) from the rest of its journey. 

In the figure below, some example of uncertainty estimate are shown below. Ideally, you wand the colored area to be small (low uncertainty), but when assessing an uncertainty, you are looking that the red cross is distributed on the map according to the colored area. That is, if the red cross is too often outside, your uncertainty is too confident, and if too often at high value, you are overconfident. Both extreme are not good.

In addition the shape of the uncertainty can be quite special (e.g., hoopoe bottom right) making the distance between the most likely point (blue) and true value (red cross) not a good measure of uncertainty. 
...
Assessing uncertainty relies on checking that that the red cross is distributed according to each uncertainty shape. A more formal way to quantifiying the uncertainty is of the quantile of the true value $q=p(\boldsymbol{x}\leq \boldsymbol{x}_{true})$, which correspond to the proability that the variable is less than or equal to the true value. So, if the true value belong to the distribution, it should have its distribution of quantile should follow a uniform distribution.

This ca be visualize with the empirical cumulatie distribution of the quantiles which should fall on the 1:1 line (like a qq-plot). The two expreme cases ($w=1$ and $w=1/n$) shows overconfidence (above the line) and  underconfidence (under the line) respectively. Indeed, the distribution of quantile using the $w=1/n$ method shows a lot of quantile value between 0.9-1, which indicates that the true value is within the 90% uncertainty contour (underconfident).


![](probability-aggregation-qq.png){width=100%}

The method chosen ($w=\log(n)/n$) is doing better than both, but is still in general underconfident. Because the calibration the validation is performed with rather long temporal serie (equipement and retrival), one can expect that the labelisation of pressure is better/easier than for other place. Thus, it seems more appropriate to be slightly underconfident while the method is being developed. 

The covariance aggregation scheme was implemented by computing the covariance matrix of the multi-variate Gaussian distribution. The covariance function was build using the exact on the variogram of each species. This is, in theory, the more correct method, but it look too overconfident. Thus we didn't use it for now.

Looking forward, calibration is highly dependant on the local topograph and the ability of the bird to move up and down. Manual edition/labeling has also a strong impact. 
