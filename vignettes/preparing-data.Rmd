...
This vignette covers important checks which should be performed to ensure that modelling with a graph is successful. 

In the vignette [Labelling tracks](./articles/labelling-tracks), we checked that the pressure timeseries measured by the geolocator are consistent with a least one location on the map. Make sure that Test #4 outlined in that vignette is carefully carried out before running the steps outlined below.

In the following vignette, we will check that pressure, light and flight duration data allow for a coherent trajectory. It is possible that some manual editing of the pressure data labelling will be required, especially for short stopovers.

We suggest starting by selecting only long stopovers (24-48hours), as this will help to draw out the general trajectory followed by the bird, and only later adding the shorter stopovers which could raise more confusion (the best match can be completely off). This is where the flight duration can help identify a realistic trajectory.

As a general guideline, use your common sense, rather than solely on the model, to draw out the trajectory course. The model needs to give realistic trajectories, otherwise there is a labelling issue. For example, a small vertical movement of the bird during a stopover can completely throw off the estimation. 
...
Start by loading the light and pressure data computed in the respective vignettes [Light map](./articles/light-map.html) and [Pressure map](./articles/pressure-map.html).
...
In this step, we need to retrieve the pressure and light data for the same stationary period. There are three sources of data to overlap, the last one (`sta_id_keep`) is where you can choose to include only part of the data. As mentioned above, it is easier to start by filtering only longer stationary periods, and then reduce the threshold on stopover duration.
...
We then need to keep all the flights between each stationary period separate so that we can estimate the wind support correctly.
...
We compute the static probability with the product of light and pressure probability maps, and add the flight duration in the metadata.
...
In this section, we will look at a few checks to perform before moving to the modelling step. These check are performed when creating the graph `graph_create()`, but for pedagogical reason, I thought it is nice to present them step by step here. 

### Check 1: Existance of positionf with static probability
...
### Check 2: Presence of at least one transition
...
save(pressure_prob,
  ...
  static_prob,
  pam_data,
  file = "geopressureviz.RData")
```


## Export static probability

It can be useful to first run the model on a downscale map to improve computational costs. This is not necessary in this example.
