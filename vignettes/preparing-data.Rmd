...
We load the light and pressure data computed in the vignette [Light map](./articles/light-map.html) and [Pressure map](./articles/pressure-map.html) respectively.
...
Then, some visual quality control are always very useful and helpful to get a good understanding of the movement of the bird. Check that the trajectory make sense based  on the transition from one stationay period to the next (look at flight duration)
...
Finally, we can check that there are always at least one transition possible from one stationary period to the next. 
