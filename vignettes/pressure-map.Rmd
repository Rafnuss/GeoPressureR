...
pressure_timeserie <- readRDS(system.file("extdata", "18LX_pressure_timeserie.rda", package = "GeoPressureR"))
...
```{r, eval=F}
pressure_timeserie_1 <- geopressure_ts(pt$lon, pt$lat, pressure = subset(pam_data$pressure, sta_id == 1))
```
...
Secondly, we can use `geopressure_ts_path()` which basically call `geopressure_ts()` in parallel for all stationary periods. We can additionally request to compute the altitude during the next flight flight with `include_flight = c(0,1)`.
...
p <- ggplot() +
 geom_line(data = do.call("rbind", pressure_timeserie), aes(x = date, y = altitude)) +
  theme_bw() + scale_y_continuous(name = "Altitude (m)")
ggplotly(p, dynamicTicks = T) %>% layout(showlegend = F)
```

`pressure_timeserie` is also used to check the correct labeling of the track (see [Labelling tracks](labelling-tracks.html)).
