...
for (i_s in seq_len(grl$sz[3]-1)){
  ...
  m <- m %>% addAwesomeMarkers(lng = (grl$shortest_path$lon[i_s] + grl$shortest_path$lon[i_s+1])/2,
...
plot1 <- ggplot(energy_df, aes(reorder(name, sta_id_s), gs)) + theme_bw() + geom_boxplot()
plot2 <- ggplot(energy_df, aes(reorder(name, sta_id_s), ws)) + theme_bw() + geom_boxplot() 
plot3 <- ggplot(energy_df, aes(reorder(name, sta_id_s), as)) + theme_bw() + geom_boxplot() 
plot4 <- ggplot(energy_df, aes(reorder(name, sta_id_s), flight_duration)) + theme_bw() +geom_point()
plot5 <- ggplot(energy_df, aes(reorder(name, sta_id_s), energy)) + theme_bw() + geom_boxplot() 
# grid.arrange(plot1, plot2, plot3, plot4, plot5, nrow=5)

subplot(ggplotly(plot1), ggplotly(plot2), ggplotly(plot3), ggplotly(plot4), ggplotly(plot5), nrows=5, titleY=T)
```
