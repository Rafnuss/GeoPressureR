...
In this vignette, we will use light data to estimate the position of the Great Reed Warbler (18IC) at each stationary period as a probability map. All functions used are a modification from the `GeoLight` package. 
...
Define basic information about the calibration period and location. The second calibration is so short (1 day), that we don't consider it in this example.
...
The manual editing is easily performed with trainset. In this case, you label the datapoint you want to delete with any label name and do not assign any label to the datapoint you want to keep (leave as black dot). Read more about trainset in the vignette [How to label tracks](labeling_tracks.html#introduction-to-trainset).
...
Instead of calibrating the twilights error in term of duration, we model directly the zenith angle error.
...
We first define a grid on which to compute the probabilities. For ease of comparison with the pressure derived map, we simply load the grid size and resolution from `prob_map_list` (computed in [How to use GeoPressureR](using-geopressurer.html#computing-the-map-of-pressure))
```{r}
...
Selecting only the twilights not labeled as deleted, we compute the probability of observing the zenith angle of each twilights using the calibrated error function for each grid cell.
...
Aggregating the probability map of each twilight per stationary periods required some assumption on the independence/correlation of the twilight errors. Read more about this in the vignette [Probability-aggregation](Probability-aggregation.html))
