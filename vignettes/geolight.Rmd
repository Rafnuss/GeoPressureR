...
You can optionally perform the automatic editing
...
for (i_s in seq_len(nrow(pam_data$sta))){
  ...
  if (sum(id)>1){
    g$p = exp(colSums(w*log(pgz[id,]))) # Log-linear equation express in log 
  } else if(sum(id)==1) {
    g$p = pgz[id,]
  } else {
    g$p = 1
...
l %>% 
  addCircles(lng=lon.calib,lat=lat.calib,color = "white", opacity=1) %>% 
  addLayersControl(
    overlayGroups = li_s,
