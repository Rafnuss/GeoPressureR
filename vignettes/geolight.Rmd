...
Before computing the probability map, we group the twilights by stationary period using activity data. Learn more about this step in [How to use GeoPressureR](using-geopressurer.html#identifying-stationary-periods).
...
We first define a grid on which to compute the probabilities. For ease of comparison with the pressure-derived map, we load the grid size and resolution from `prob_map_list` (computed in [How to use GeoPressureR](using-geopressurer.html#computing-the-map-of-pressure))
```{r}
